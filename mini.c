...
/* This was in the "tokenizer" section */
enum {
	DO_SYM,
	ELSE_SYM,
	IF_SYM,
...
/* This was in the "parser" section */
enum {
	VAR,
	CST,
	ADD,
...
/** It appears this "node" structure is what the interpreter will read */
typedef struct node {
	int kind;
...
/** This was in the "interpreter" section; not sure what it is yet */
typedef struct list {
	char *id;
	int value;
	struct list *next;
...
/*------------------------------------------------------------------------*/
/* TO BE SORTED (original author's code, organized VERY differently lol)  */
/*------------------------------------------------------------------------*/
...
int main() {
	printf("A mini language in C\nType some code here:  ");
	
	/* Step 1: figure out what exactly is a "node" */
	node *prog = parse();
